...
async def connect_to_db():
    conn = pymysql.connect(host='db', user='payhere', password='payhere', port=3306, charset='utf8', db='payhere')
    return conn
...
# 시간대 설정
def convert_kst():
    dt_tm_utc = datetime.strptime(time.strftime('%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S')
