...
async def copy_account_detail(email: str, account_index: int) -> Response:
    try:
        ...
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            if result['note'] is not None: sql_data['note'] = result['note'] 
            sql += "null," if result['type'] is None else "%(type)s,"
            if result['type'] is not None: sql_data['type'] = result['type']
...
async def create_account_detail_url(email: str, account_index: int, data) -> Response:
    try:
        conn = await connect_to_db()
        if data.create_time is False and data.last_update_time is False and data.money is False and data.note is False and data.type is False:
...
async def get_account_detail_by_url(email: str, url: str) -> Response:
    try:
        ...
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            if result is None:
                return Response(status=False, message='가계부 세부 내역을 찾을 수 없습니다.', data=None)
        
            response_result = dict()
            if result['u.create_time'] == 1: response_result['create_time'] = result['create_time']
            if result['u.last_update_time'] == 1: response_result['last_update_time'] = result['last_update_time']
            if result['u.money'] == 1: response_result['money'] = result['money']
            if result['u.note'] == 1: response_result['note'] = result['note']
            if result['u.type'] == 1: response_result['type'] = result['type']
            
            response_result = json.dumps(response_result, default=str, ensure_ascii=False)
